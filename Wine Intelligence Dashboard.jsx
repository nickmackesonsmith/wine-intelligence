...
// Used to power personalised recommendations — not displayed as a cellar view
...
// ─── Constants ──────────────────────────────────────────────────────────────
const COLOUR_MAP = {
  red: { bg: "bg-red-50", border: "border-red-200", accent: "bg-red-600", text: "text-red-700", chart: "#991b1b", light: "#fef2f2" },
  white: { bg: "bg-amber-50", border: "border-amber-200", accent: "bg-amber-500", text: "text-amber-700", chart: "#d97706", light: "#fffbeb" },
  "rosé": { bg: "bg-pink-50", border: "border-pink-200", accent: "bg-pink-400", text: "text-pink-700", chart: "#db2777", light: "#fdf2f8" },
  sparkling: { bg: "bg-yellow-50", border: "border-yellow-200", accent: "bg-yellow-400", text: "text-yellow-700", chart: "#ca8a04", light: "#fefce8" },
  fortified: { bg: "bg-purple-50", border: "border-purple-200", accent: "bg-purple-600", text: "text-purple-700", chart: "#7c3aed", light: "#faf5ff" },
  orange: { bg: "bg-orange-50", border: "border-orange-200", accent: "bg-orange-500", text: "text-orange-700", chart: "#ea580c", light: "#fff7ed" },
};
const TABS = [
  { id: "explore", label: "Explore Varietals" },
...
const PROFILE_KEYS = [
  ...
  { key: "sweetness", label: "Sweetness" }, { key: "fruit", label: "Fruit" }, { key: "floral", label: "Floral" },
  { key: "spice", label: "Spice" }, { key: "earth", label: "Earth" }, { key: "oak", label: "Oak" },
  { key: "alcohol", label: "Alcohol" }, { key: "complexity", label: "Complexity" },
];
...
function VarietalCard({ varietal, onSelect, isSelected, rating, onRate }) {
  const c = COLOUR_MAP[varietal.colour] || COLOUR_MAP.red;
  const history = DRINKING_HISTORY[varietal.name];
  const totalDrunk = history ? history.drank : 0;
  return (
    <div className={`rounded-lg border-2 p-4 cursor-pointer transition-all hover:shadow-md ${isSelected ? `${c.bg} ${c.border} shadow-md` : "bg-white border-gray-200 hover:border-gray-300"}`} onClick={() => onSelect(varietal)}>
      <div className="flex items-start justify-between mb-2">
        <div>
          <h3 className="font-semibold text-gray-900 text-base">{varietal.name}</h3>
          <div className="flex items-center gap-2 mt-1">
            <span className={`inline-block text-xs px-2 py-0.5 rounded-full text-white ${c.accent}`}>{varietal.colour}</span>
            {totalDrunk > 0 && <span className="text-xs text-gray-400">{totalDrunk} bottle{totalDrunk !== 1 ? "s" : ""} logged</span>}
          </div>
        </div>
      </div>
      <p className="text-sm text-gray-600 line-clamp-2 mb-2">{varietal.description}</p>
      <div onClick={(e) => e.stopPropagation()}><StarRating rating={rating || 0} onRate={(r) => onRate(varietal.id, r)} /></div>
    </div>
  );
}

function VarietalDetail({ varietal, rating, onRate }) {
  const c = COLOUR_MAP[varietal.colour] || COLOUR_MAP.red;
  return (
    <div className={`rounded-xl border-2 ${c.border} ${c.bg} p-6`}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{varietal.name}</h2>
          <p className="text-sm text-gray-500 mt-1">{varietal.regions}</p>
        </div>
        <StarRating rating={rating || 0} onRate={(r) => onRate(varietal.id, r)} />
      </div>
      <div className="grid grid-cols-2 gap-6">
        <RadarProfile varietal={varietal} height={300} />
        <div className="space-y-4">
          <div><h4 className="font-semibold text-gray-800 text-sm uppercase tracking-wide mb-1">About</h4><p className="text-sm text-gray-700 leading-relaxed">{varietal.description}</p></div>
          <div><h4 className="font-semibold text-gray-800 text-sm uppercase tracking-wide mb-1">Tasting Notes</h4><p className="text-sm text-gray-700 leading-relaxed">{varietal.tastingNotes}</p></div>
          <div><h4 className="font-semibold text-gray-800 text-sm uppercase tracking-wide mb-1">Food Pairing</h4><p className="text-sm text-gray-700 leading-relaxed">{varietal.foodPairing}</p></div>
        </div>
      </div>
      <div className="mt-6">
        <h4 className="font-semibold text-gray-800 text-sm uppercase tracking-wide mb-3">Buy in Auckland</h4>
        <div className="grid grid-cols-2 gap-2">
          {varietal.recommendations.map((rec, i) => (
            <a key={i} href={rec.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-lg bg-white border border-gray-200 hover:border-gray-400 hover:shadow-sm transition-all group">
              <div><p className="text-sm font-medium text-gray-900 group-hover:text-blue-600">{rec.name}</p><p className="text-xs text-gray-500">{rec.shop}</p></div>
...
function CompareView({ varietals, ratings, onRate }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {w1 && w2 && (
        <div className="space-y-6">
          <RadarProfile varietal={w1} compareTo={w2} height={350} />
          <div className="grid grid-cols-2 gap-6">
...
// ─── What to Buy ────────────────────────────────────────────────────────────
function WhatToBuy({ varietals, ratings, onRate, onNavigate }) {
  const [buyFilter, setBuyFilter] = useState("all");

  const recommendations = useMemo(() => {
    const recs = [];

    // 1. "Restock favourites" — varietals you've drunk a lot but don't have in cellar
    ...
    restockCandidates.forEach((v) => {
      ...
      v.recommendations.slice(0, 2).forEach((rec) => {
        recs.push({
          ...rec, varietal: v, category: "restock",
          reason: `You've had ${h.drank} bottles of ${v.name} and have none left. Time to restock.`,
          priority: h.drank,
        });
      });
    });

    // 2. "On your wishlist" — varietals with wishlist items
    const wishlistVarietals = VARIETALS.filter((v) => {
      const h = DRINKING_HISTORY[v.name];
      return h && h.wishlist > 0;
    });
    wishlistVarietals.forEach((v) => {
      const h = DRINKING_HISTORY[v.name];
      v.recommendations.slice(0, 1).forEach((rec) => {
        recs.push({
          ...rec, varietal: v, category: "wishlist",
          reason: `${h.wishlist} bottle${h.wishlist > 1 ? "s" : ""} on your wishlist. You know you want ${v.name === "Pinot Noir" ? "more Pinot" : "this"}.`,
          priority: h.wishlist + (h.drank || 0),
        });
      });
    });

    // 3. "Expand your horizons" — varietals you haven't tried yet
    const unexplored = VARIETALS.filter((v) => {
      const h = DRINKING_HISTORY[v.name];
      return !h || (h.drank === 0 && h.inCellar === 0 && h.wishlist === 0);
    });
    unexplored.forEach((v) => {
      v.recommendations.slice(0, 1).forEach((rec) => {
        recs.push({
          ...rec, varietal: v, category: "explore",
          reason: v.description,
          priority: v.profile.complexity,
        });
      });
    });

    // 4. "Level up" — you've tried it once or twice, go deeper
    const levelUp = VARIETALS.filter((v) => {
      const h = DRINKING_HISTORY[v.name];
      return h && h.drank >= 1 && h.drank <= 2 && h.inCellar === 0;
    });
    levelUp.forEach((v) => {
      const h = DRINKING_HISTORY[v.name];
      const triedProducers = h.producers.join(", ");
      v.recommendations.filter((rec) => !h.producers.some((p) => rec.name.includes(p))).slice(0, 1).forEach((rec) => {
        recs.push({
          ...rec, varietal: v, category: "level-up",
          reason: `You've tried ${v.name} from ${triedProducers}. Here's a new producer to compare.`,
          priority: v.profile.complexity,
        });
      });
    });
  ...
  const categoryInfo = {
    restock: { label: "Restock Favourites", emoji: "♻️", desc: "Wines you love but don't have in the fridge", colour: "bg-red-50 border-red-200 text-red-700" },
    wishlist: { label: "From Your Wishlist", emoji: "⭐", desc: "Already on your radar — pull the trigger", colour: "bg-blue-50 border-blue-200 text-blue-700" },
    "level-up": { label: "Level Up", emoji: "📈", desc: "You've tried it — now go deeper with a new producer", colour: "bg-green-50 border-green-200 text-green-700" },
    explore: { label: "Expand Your Horizons", emoji: "🌍", desc: "Varietals you haven't explored yet", colour: "bg-purple-50 border-purple-200 text-purple-700" },
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">Based on your 66-bottle drinking history — what you love, what you're missing, and what to try next.</p>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setBuyFilter("all")} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${buyFilter === "all" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>All ({recommendations.length})</button>
          {categoryOrder.map((cat) => {
            const count = recommendations.filter((r) => r.category === cat).length;
            if (count === 0) return null;
            return <button key={cat} onClick={() => setBuyFilter(cat)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${buyFilter === cat ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{categoryInfo[cat].emoji} {categoryInfo[cat].label} ({count})</button>;
          })}
        </div>
      </div>
        ...
        return (
          <div key={cat}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-gray-400">— {info.desc}</span>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {items.sort((a, b) => b.priority - a.priority).map((rec, i) => {
                const c = COLOUR_MAP[rec.varietal.colour] || COLOUR_MAP.red;
                return (
                  <div key={`${cat}-${i}`} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-all">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <h4 className="font-semibold text-gray-900 text-base mb-1">{rec.name}</h4>
                        <p className="text-sm text-gray-500">{rec.reason}</p>
                      </div>
                      <div className="flex gap-2 ml-4">
                        <button onClick={() => onNavigate(rec.varietal)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors whitespace-nowrap">View profile</button>
                        <a href={rec.url} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-900 text-white hover:bg-gray-800 transition-colors whitespace-nowrap">Buy →</a>
                      </div>
                    </div>
                  </div>
                );
...
export default function WineDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Wine Intelligence</h1>
        <p className="text-gray-500 mt-1">Your personal sommelier — 27 varietals profiled, personalised to your palate</p>
      </div>
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-6 w-fit">
        {TABS.map((tab) => (
          <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSelectedVarietal(null); }} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === tab.id ? "bg-white shadow text-gray-900" : "text-gray-600 hover:text-gray-900"}`}>{tab.label}</button>
        ))}
      </div>
      ...
      {activeTab === "explore" && (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex gap-1">
              {FILTERS.map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filter === f ? "bg-gray-900 text-white" : "bg-white text-gray-600 border border-gray-200 hover:border-gray-400"}`}>{f}</button>
              ))}
            </div>
            <input type="text" placeholder="Search varietals..." className="flex-1 min-w-48 border border-gray-300 rounded-lg px-3 py-1.5 text-sm" value={search} onChange={(e) => setSearch(e.target.value)}/>
          </div>
          {selectedVarietal ? (
            <div>
              <button onClick={() => setSelectedVarietal(null)} className="text-sm text-gray-500 hover:text-gray-700 mb-3 flex items-center gap-1">← Back to all varietals</button>
              <VarietalDetail varietal={selectedVarietal} rating={ratings[selectedVarietal.id]} onRate={handleRate} />
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-4">
              {filteredVarietals.map((v) => <VarietalCard key={v.id} varietal={v} onSelect={setSelectedVarietal} isSelected={false} rating={ratings[v.id]} onRate={handleRate} />)}
            </div>
          )}
        </div>
      )}

      {activeTab === "buy" && <WhatToBuy varietals={VARIETALS} ratings={ratings} onRate={handleRate} onNavigate={navigateToVarietal} />}
